...
program
    ...
    .option('-m, --with-source-maps', 'output source maps when not generating a bundled app')
...
var make_lib_files = function (import_format, source_maps, with_app_dir) {
    ...
    // commonjs (and umd) support properly shifting the default export to work as normal
    if (import_format === 'commonjs' || import_format === 'umd') {
        babel_opts.plugins.unshift("add-module-exports");
    }
    ...
    if (with_app_dir) {
        ...
        if (helpers[import_format]) {
            console.log(`Writing ${out_app_path}`);
            let out_script = helpers[import_format](out_path_base, out_app_path);
